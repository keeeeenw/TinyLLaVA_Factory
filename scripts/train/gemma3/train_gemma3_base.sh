...
LLM_VERSION=google/gemma-3-270m # llm path in huggingface
...
CONV_VERSION=gemma #chat template, other options are: phi, llama, gemmma, etc
VERSION=gemma3-270m_base #experiment name for recording different runnings
TRAIN_RECIPE=common #training recipes, other options are: lora, qlora
MODEL_MAX_LENGTH=2048 #max model length for llm

# Note: --attn_implementation eager is recommended. Otherwise, there will be a warning message.
bash scripts/train/gemma3/pretrain_gemma3.sh "$DATA_PATH" "$IMAGE_PATH" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH"
# bash scripts/train/gemma3/finetune_gemma3.sh "$FINETUNE_DATA_PATH" "$FINETUNE_IMAGE_PATH" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$CONV_VERSION" "$VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH"
